...
public class GlideUtil {
    ...
    public static void loadImageView(Context mContext, String path, ImageView mImageView) {
        Glide.with(mContext).load(path).diskCacheStrategy(DiskCacheStrategy.SOURCE).placeholder(R.mipmap.loading).error(R.mipmap.loading).into(mImageView);
    }

    //默认平铺加载
    public static void loadImageViewCenterCrop(Context mContext, String path, ImageView mImageView) {
        Glide.with(mContext).load(path).diskCacheStrategy(DiskCacheStrategy.SOURCE).placeholder(R.mipmap.loading).error(R.mipmap.loading).centerCrop().into(mImageView);
    }

    //加载本地
    public static void loadImageView(Context mContext, int resourceId, ImageView mImageView) {
        Glide.with(mContext).load(resourceId).diskCacheStrategy(DiskCacheStrategy.SOURCE).placeholder(R.mipmap.loading).error(R.mipmap.loading).into(mImageView);
    }
    ...
    public static void loadCircleImageView(Context mContext, String path, ImageView mImageView, int lodingImage, int errorImageView) {
        Glide.with(mContext).load(path).transform(new GlideCircleTransform(mContext)).placeholder(lodingImage).error(errorImageView).into(mImageView);
    }
    //加载圆形图片
    public static void loadCircleImageView(Context mContext, String path, ImageView mImageView) {
        Glide.with(mContext).load(path).transform(new GlideCircleTransform(mContext)).into(mImageView);
    }
    //加载圆角图片
    public static void loadRoundImageView(Context mContext, String path, ImageView mImageView) {
        Glide.with(mContext).load(path).transform(new GlideRoundTransform(mContext)).into(mImageView);
    }
    //加载圆角图片
    public static void loadRoundImageView(Context mContext, Uri path, ImageView mImageView) {
        Glide.with(mContext).load(path).transform(new GlideRoundTransform(mContext)).into(mImageView);
    }
